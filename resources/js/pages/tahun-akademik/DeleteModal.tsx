...
export default function DeleteModal({ isOpen, onClose, academicYear }: Props) {
  ...
  const handleSubmit = (e: FormEvent) => {
    e.preventDefault();
    destroy(`/tahun-akademik/${academicYear.id}`, {
      onSuccess: () => {
        toast.success("Data tahun akademik berhasil dihapus!");
        onClose();
      },
      onError: () => {
        toast.error("Gagal menghapus data tahun akademik.");
      }
    });
  };
      ...
      <div className="relative bg-background w-full max-w-md rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.12)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.30)] overflow-hidden flex flex-col border border-border/50 animate-in fade-in zoom-in duration-200 m-4">
        <div className="p-6 border-b border-border/50 flex items-center justify-between bg-red-500/10 dark:bg-red-500/5">
          <div className="flex items-center gap-3 text-red-600 dark:text-red-500">
            ...
            <h3 className="text-xl font-bold tracking-tight">Hapus Tahun Akademik</h3>
          </div>
          <button 
            type="button"
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="flex flex-col">
          <div className="p-6">
            <p className="text-muted-foreground text-sm leading-relaxed">
              Apakah Anda yakin ingin menghapus tahun akademik <span className="font-bold text-foreground">{academicYear.year}</span>? 
