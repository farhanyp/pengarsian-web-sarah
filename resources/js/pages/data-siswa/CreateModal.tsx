...
export default function CreateModal({ isOpen, onClose, availableClasses, availableAcademicYears }: Props) {
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center">
      ...
      <div className="relative bg-background w-full max-w-lg rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.12)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.30)] overflow-hidden flex flex-col border border-border/50 animate-in fade-in zoom-in duration-200 m-4">
        <div className="p-6 border-b border-border/50 flex items-center justify-between bg-muted/20">
          <h3 className="text-xl font-bold tracking-tight text-foreground">Tambah Data Siswa</h3>
          <button 
            type="button"
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="flex flex-col">
          <div className="p-6 space-y-5 max-h-[70vh] overflow-y-auto">
            <div>
              <label className="block text-sm font-semibold text-foreground mb-1.5">NISN</label>
              <input
                type="text"
                value={data.nisn}
                ...
                required
              />
              {errors.nisn && <p className="text-red-500 text-xs mt-1.5 font-medium">{errors.nisn}</p>}
            </div>

            <div>
              <label className="block text-sm font-semibold text-foreground mb-1.5">NIS</label>
              <input
                type="text"
                value={data.nis}
                ...
                required
              />
              {errors.nis && <p className="text-red-500 text-xs mt-1.5 font-medium">{errors.nis}</p>}
            </div>
            ...
            <div>
              ...
              <SearchableSelect
                ...
                value={data.class_id}
                onChange={(val) => setData('class_id', val as string)}
            ...
            <div>
              ...
              <SearchableSelect
                ...
                onChange={(val) => setData('academic_year', val as string)}
